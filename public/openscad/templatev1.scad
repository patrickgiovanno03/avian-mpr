...
odd_height = 10.5; // [1:0.5:10]

// Height for even letters
even_height = 7; // [1:0.5:10]
...
if (show_ring) {
    ...
    ring_y_pos = get_ring_y_offset(text[0]);

    difference() {

        // Donut
        difference() {
            translate([ring_x_pos, ring_y_pos, 2])
                cylinder(h = ring_height, d = keychainHoleSize + 5);

            translate([ring_x_pos, ring_y_pos, 1])
                cylinder(h = ring_height + 2, d = keychainHoleSize);
        }
...
my_scale = len(text) > 10 ? 0.9 : 1;

// Letters
scale([my_scale, my_scale, 1])
for (i = [0 : len(text) - 1]) {
